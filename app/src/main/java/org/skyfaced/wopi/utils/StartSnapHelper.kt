...
class StartSnapHelper(private val offset: Int = 0) : LinearSnapHelper() {
    ...
    override fun calculateDistanceToFinalSnap(
        ...
        if (layoutManager.canScrollHorizontally()) {
            out[0] = distanceToStart(targetView, getHorizontalHelper(layoutManager)) - offset
        } else {
            out[0] = 0
        }
        if (layoutManager.canScrollVertically()) {
            out[1] = distanceToStart(targetView, getVerticalHelper(layoutManager)) - offset
        } else {
            out[1] = 0
        }
        return out
